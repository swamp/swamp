...
fn test_layout_size_and_alignment() {
    ...
    let expected = 2 * size_of::<u16>()
        + 2 * capacity as usize * size_of::<u16>()
        + capacity as usize * size_of::<u16>()
        + ((4
            - ((2 * size_of::<u16>()
                + 2 * capacity as usize * size_of::<u16>()
                + capacity as usize * size_of::<u16>())
                % 4))
            % 4)
        + capacity as usize * element_size as usize;
...
fn test_element_count_and_slot_to_id_loop() {
    ...
    unsafe {
        ...
        for &v in &values {
            ...
            insert(base, id, element_size, &raw const v);
            assert!(is_alive(base, id, generation));
        }
