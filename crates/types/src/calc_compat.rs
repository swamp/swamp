...
impl Type {
    ...
    pub fn compatible_with_storage_and_view(
        ...
        if matches!(&*self.kind, TypeKind::StringView(..))
            && matches!(
                &*other_view.kind,
            ...
            )
        {
            match &*other_view.kind {
                TypeKind::VecStorage(inner_type, ..)
        ...
        if matches!(&*self.kind, TypeKind::StringStorage(..))
            && matches!(&*other_view.kind, TypeKind::StringView(inner_type, ..))
        {
            //return false;
        }
