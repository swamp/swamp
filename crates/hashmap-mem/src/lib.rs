...
pub const fn is_power_of_two(n: usize) -> bool {
    n > 0 && n.is_power_of_two() // More efficient power of two check
}
...
pub unsafe fn init(map_base: *mut u8, config: &MapInit) { unsafe {
    debug_assert!(
        is_power_of_two(config.capacity as usize),
...
#[inline]
pub unsafe fn get_or_reserve_entry(base_ptr: *mut u8, key_ptr: *const u8) -> *mut u8 { unsafe {
    ...
    debug_assert!(is_power_of_two(capacity), "Capacity must be a power of two");
...
/// - `key_ptr` must point to a valid key of the size specified in the map header
#[inline]
#[must_use] pub unsafe fn has(base_ptr: *const u8, key_ptr: *const u8) -> bool { unsafe {
...
#[inline]
pub unsafe fn lookup(base_ptr: *mut u8, key_ptr: *const u8) -> *mut u8 { unsafe {
    let header = &*base_ptr.cast::<MapHeader>();
...
#[inline]
pub unsafe fn remove(base_ptr: *mut u8, key_ptr: *const u8) -> bool { unsafe {
    let header = &*base_ptr.cast::<MapHeader>();
...
#[inline]
pub unsafe fn overwrite(target_base: *mut u8, source: *const u8) -> bool { unsafe {
