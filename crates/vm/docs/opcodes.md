...
- R0: Return register
- R1-R5. Argument registers. Caller-saved.
- R6-R127: Callee-saved registers. E.g. local function variables.
...
| `blk.cpy`                  | `Rd, Rn, Rm`                    | Copy block of memory with size in register $Rm$ from address $Rn$ to address $Rd$.                                                      |
...
| **Vector Operations**      |                                 |                                                                                                                                         |
| `vec.push`                 | `Rd, Rm, #element_size`         | Append space for an element to vector $Rm$ and return the address in $Rd$.                                                              |
| `vec.rem`                  | `Rd, Rm, #element_size`         | Remove the element at index $Rm$ from vector $Rd$.                                                                                      |
| `vec.pop`                  | `Rd, Rm, #element_size`         | Remove the last element from vector $Rm$ and store its address in $Rd$.                                                                 |
| `vec.rem.v`                | `Rd, Rm, Rp`                    | Remove the element at index $Rp$ from vector $Rm$ and store its value in $Rd$.                                                          |
| `vec.get`                  | `Rd, Rm, Rp, #element_size`     | Get the element at index $Rp$ from vector $Rm$ and store its address in $Rd$.                                                           |
...
| `vec.iter`                 | `Rd, Rm, #element_size`         | Initialize a vector iterator for vector $Rm$, store iterator state in $Rd$.                                                             |
...
| `grid.entry_addr`          | `Rd, Rm, Rp, Rq, #element_size` | Get the entry address at coordinates ($Rp$,$Rq$) in grid $Rm$, store in $Rd$.                                                           |
