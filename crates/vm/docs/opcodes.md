...
| `tst`                             | 20       | `Rn`                        | Tests the value of register $Rn$ and updates the predicate flag. If $Rn$ is zero, predicate flag will be zero (unset), otherwise predicate flag is one (set).                     |
| `notp`                            | 22       |                             | Inverts the state of the predicate flag. **TODO: should be removed?**.                                                                                                            |
| `movp`                            | 23       | `Rd`                        | Stores the state of the predicate flag into register $Rd$.                                                                                                                        |
| `movnp`                           | 24       | `Rd`                        | Stores the inverse state of the predicate flag into register $Rd$. **TODO: should be removed?**.                                                                                  |
| **Move and Copy**                 |          |                             |                                                                                                                                                                                   |
...
| `ld.b`                            | ?        | `Rd, #absolute_addr`        | Loads a 8-bit value from the absolute memory address `#absolute_addr` into register $Rd$. Used mainly for constants placed at known locations.                                    |
| `ld.h`                            | ?        | `Rd, [Rn, #offset]`         | Loads an unsigned 16-bit value from memory at address $Rn$ + `#offset` into register $Rd$.                                                                                        |
| `ld.h`                            | ?        | `Rd, #absolute_addr`        | Loads an unsigned 16-bit from the absolute memory address `#absolute_addr` into register $Rd$. Used mainly for constants placed at known locations.                               |
| `ld.w` (alias `ld`)               | 37       | `Rd, [Rn, #offset]`         | Loads a 32-bit value from memory at address $Rn$ + `#offset` into register $Rd$.                                                                                                  |
| `ld.w` (alias `ld`)               | ?        | `Rd, #absolute_addr`        | Loads a 32-bit value from the absolute memory address `#absolute_addr` into register $Rd$. Used mainly for constants placed at known locations.                                   |
...
| `vec.iter.next`                   | 71       | `#branch_delta, Rd, Rn`     | Advances the vector iterator in $Rn$, stores the next element value in $Rd$, and updates $Rn$. Branches to PC + `#offset` if iteration is complete.                               |
| `vec.iter.next.pair`              | 72       | `#branch_delta, Rd, Rn`     | Advances the vector iterator in $Rn$, stores the next index-value pair in $Rd$ (index) and $Rd+1$ (value), and updates $Rn$. Branches to PC + `#offset` if iteration is complete. |
...
| `map.iter.next.pair`              | 86       | `#branch_delta, Rd, Rn`     | Advances the map iterator in $Rn$, stores the next key-value pair in $Rd$ (key) and $Rd+1$ (value), and updates $Rn$. Branches to `#branch_delta` if iteration is complete.       |
