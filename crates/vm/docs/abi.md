...
The ABI defines the low-level conventions for how compiled code
interacts with the VM runtime, including function calls, data representation,
and register usage.

Adhering to this ABI is essential for compilers and runtime
libraries targeting the Swamp VM to ensure correct interoperation.
...
The Swamp VM has 256 general-purpose registers ($R0$ - $R255$), but only
$R0$-$R127$ are currently used. This ABI defines specific roles and save
...
- **R0 (Return Value Register):**
  ...
  - If a function returns a value, the result is placed in $R0$ by the callee
    before returning, or if it is a indirect value, the pointer stored in the
    $R0$ is used.
...
- **$R7$ - $R95$ (Callee-Save / Frame Registers):**

  - This range of **89 registers** ($R7$ through $R95$) is designated as
    **Callee-Save**.
...
- **$R96$ - $R127$ (Caller-Save / Temporary Registers - Primary Range):**

  - This range of 32 registers is designated as **Caller-Save**.

  - Callee functions are free to modify these registers ($R96$ through $R127$)
...
1. **Argument & Hidden Pointer Passing:**
   ...
   - **Indirect Values. For functions returning values larger than 32 bits
     ...
     - Place the second explicit argument in $R2$, ..., up to the fifth explicit
       argument in $R5$.
...
3. **Callee-Save Registers:** If the function needs to use any registers in the
   **$R7$-$R95$** range, it must save their current values (typically onto the
...
4. **Local Variables:** Allocate space for local variables within the stack
   ...
   made by this function are good candidates for $R1$-$R96$ registers.

5. **Body Execution:** Execute the function's logic.

   - **For functions returning Direct Values, 32-bit or smaller values:**
     Compute the result and place it into $R0$ before returning.

   - **For functions returning values Indirect Values, larger than 32 bits:**
...
6. **Restore Callee-Save Registers:** Before returning, restore the original
   values of any **R7-R95** registers that were saved upon entry.
