...
impl Vm {
    ...
    pub fn execute_vec_from_slice(&mut self, target_vec_pointer: u16, source_slice_addr: u16) {
        ...
        let heap_offset: u32 = self.heap.heap_allocate(slice_size);

        // Copy the slice from Frame to Heap.
        unsafe {
            let dest_ptr = self.heap.get_heap_ptr(heap_offset as usize);
            let src_ptr = self
                .heap
        ...
        let header_offset: u32 = self.heap.heap_allocate(VEC_HEADER_SIZE.0 as usize);

        // Copy the Vec header to Heap.
        unsafe {
            let header_ptr = self.heap.get_heap_ptr(header_offset as usize) as *mut VecHeader;
            ptr::write(header_ptr, vec_header);
        }

        // Copy the heap offset of the vec header to the frame.
        unsafe {
            let target_ptr = self.frame.get_frame_ptr_as_u32(target_vec_pointer);
    ...
    pub fn execute_vec_iter_init(&mut self, target_iterator_addr: u16, vec_indirect: u16) {
        let vec_header_heap_ptr_as_offset = self.frame.read_heap_offset_via_frame(vec_indirect);
        unsafe {
    ...
    pub fn vec_header_from_indirect_heap_mut(&self, frame_offset: u16) -> *mut VecHeader {
        let heap_offset = self.frame.read_heap_offset_via_frame(frame_offset);
        self.heap.get_heap_const_ptr(heap_offset as usize) as *mut VecHeader
    }

    #[inline]
    pub fn execute_vec_len(&mut self, int_target: u16, frame_source: u16) {
        let vec_header = Self::vec_header_from_indirect_heap(&self.frame, frame_source, &self.heap);
        unsafe {
            *self.frame.get_frame_ptr_as_i32(int_target) = vec_header.count as i32;
        }
        #[cfg(feature = "debug_vm")]
        {
    ...
    pub fn execute_vec_get(&mut self, item_target: u16, vec_indirect_source: u16, int_index: u16) {
        let vec_index = self.frame.read_frame_i32(int_index) as usize;
        let vec_header =
            Self::vec_header_from_indirect_heap(&self.frame, vec_indirect_source, &self.heap);
        debug_assert!(
            vec_index < vec_header.count as usize,
            "out of bounds for vector. index:{vec_index} out of {}",
            vec_header.count
        );
        let item_target_ptr = self.frame.get_frame_ptr(item_target);
        unsafe {
            let base_ptr = self
                .heap
    ...
    pub fn execute_vec_set(&mut self, vec_indirect_source: u16, int_index: u16, item_source: u16) {
        ...
        let item_source_ptr = self.frame.get_frame_const_ptr(item_source);
        unsafe {
            let base_ptr = self.heap.get_heap_ptr(vec_header.heap_offset as usize);
            ptr::copy_nonoverlapping(
                item_source_ptr,
    ...
    pub fn execute_vec_iter_next_pair(
        ...
        let vec_iterator = self.frame.get_frame_ptr(target_iterator_addr) as *mut VecIterator;
        ...
        if index == vec_header.count {
            ...
            let source = self.heap.get_heap_const_ptr(heap_data_offset as usize);
            let index_ptr = self.frame.get_frame_ptr_as_i32(key_variable);
            let target_ptr = self.frame.get_frame_ptr(value_variable);
    ...
    pub fn execute_vec_iter_next(
        ...
        if index == vec_header.count {
            ...
            let heap_data_offset = data_heap_offset + vec_header.element_size as u32 * index as u32;
            let source = self.heap.get_heap_const_ptr(heap_data_offset as usize);
            let target_ptr = self.frame.get_frame_ptr(target_variable);
