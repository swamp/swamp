...
use swamp_types::{Type, TypeId, TypeKind, TypeRef};
...
impl LayoutCache {
    pub fn layout(&mut self, analyzed_type: TypeRef) -> BasicTypeRef {
        if let Some(x) = self.id_to_layout.get(&analyzed_type.id) {
            ...
            let basic_type_ref = Rc::new(basic_type);

            self.id_to_layout.insert(analyzed_type.id, basic_type_ref.clone()).expect("should work");
    ...
    #[allow(clippy::too_many_lines)]
    #[must_use]
    fn layout_type(&self, ty: &TypeRef) -> &BasicType {
        match &*ty.kind {
            ...
            TypeKind::Tuple(types) => layout_tuple(types),
            ...
            TypeKind::Enum(a) => layout_enum(
                &a.assigned_name,
                &a.variants.values().cloned().collect::<Vec<_>>(),
            ),
            TypeKind::Optional(inner_type) => layout_optional_type(inner_type),
            TypeKind::MutableReference(inner_type) => layout_mutable_reference(inner_type),
            // ----------
